...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (data) {
                swal({
                    ...
                    .then((value) => {
                        window.location = "{{route('member.dashboard.index')}}";
                    });
    ...
    $(document).ready(function () {
        $(".plus11").click(function () {
            ...
            tr +=
                '@foreach ($Animal_common_name as $r) <option value="{{$r->common_name_en }}">{{ $r->common_name_en }}</option>@endforeach';
            tr += '</datalist>';
            tr += '</td>';
            tr += '<td class="p-0">';
            tr +=
            ...
            tr +=
                '@foreach ($Animal_common_name as $r) <option value="{{$r->genetic_diversty }}">{{ $r->genetic_diversty }}</option>@endforeach';
            tr += '</datalist>';
            tr += '</td>';
            tr += '<td class="p-0">';
            tr +=
            ...
            tr +=
                '@foreach ($Animal_stock as $r) <option value="{{$r->name }}">{{ $r->name }}</option>@endforeach';
            tr += '</datalist>';
            tr += '</td>';
            tr +=
                '<td class="p-0"><input type="text" class="form-control form-control-sm border-0 rounded-0" name="Age[]" required></td>';
            tr +=
                '<td class="p-0"><input type="text" class="form-control form-control-sm border-0 rounded-0" name="Weight[]" required></td>';
            tr += '<td class="p-0">';
            tr +=
            ...
            tr += '</td>';
            tr +=
                '<td class="p-0"><input type="text" class="form-control form-control-sm border-0 rounded-0 Number11" name="Number[]" required></td>';
            tr +=
                '<td class="text-center p-0"><button type="button" class="btn btn-secondary rounded-0 minus11">-</button></td>';
            tr += '</tr>';
    ...
    $('#Protocol_opic_Termination').on('submit', function (e) {
        e.preventDefault();
        var formData = new FormData(this);
        $.ajax({
            url: "{{route('member.edit.termination')}}",
            data: formData,
            type: "POST",
            async: false,
            contentType: false,
    ...
    $('#AmendmentProtocol').on('submit', function (e) {
        ...
        var formData = new FormData(this);
        $.ajax({
            url: "{{route('member.protocol.Amendment')}}",
            data: formData,
            type: "POST",
            async: false,
            contentType: false,
...
<script>
    ...
    $('.progress-002').on('change', function (e) {
        var v001 = $(".progress-001").val();
        var v002 = $(".progress-002").val();
        $(".progress-003").val(v001-v002);
    });
